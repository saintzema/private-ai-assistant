...
export default function ChatList({ chats, activeChatId, workspaceId, onDelete }: Props) {
  const router = useRouter();

  const handleDelete = async (e: React.MouseEvent, chatId: string) => {
    e.stopPropagation();
    if (!confirm("Delete this chat?")) return;
    try { await chatsApi.delete(workspaceId, chatId); onDelete(chatId); }
    ...
    <div className="py-2">
      {Object.entries(grouped).map(([label, items]) => (
        <div key={label}>
          ...
          {items.map(chat => (
            <button key={chat.id} onClick={() => router.push(`/workspace/${workspaceId}/chat/${chat.id}`)}
              className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-left group transition-colors ${activeChatId === chat.id ? "bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400" : "hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300"}`}>
